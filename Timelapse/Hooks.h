...
#pragma unmanaged
	...
	CodeCave(MpHackStartHook) {
		pushad
			cmp ebx, 00000001
			popad

			jmp dword ptr[mpHackAddrRet]
	} EndCodeCave;

	CodeCave(MpHackEndHook) {
		pushad
			cmp ebx, 00002710
			popad
	...
	CodeCave(PetItemVacHook) {
		; Replicate overidden operations(5 bytes long)
			push eax
			push edi
			lea eax, [ebp - 0x34]
			; Replication end

			; Check the status of PetGetItemSwitch
			cmp dword ptr[PetGetItemSwitch], 0
			je PetItemVacDisabled
